...
def crawl_links(start_url):
    ...
    while queue and len(urls) < MAX_PAGES:
        ...
        try:
            ...
            r = requests.get(current, timeout=15, headers=HEADERS)
...
if not all_urls:
    ...
    print("No sitemap found. Using fallback crawler.")

    all_urls = crawl_links(website)

print("\nURLs discovered:", len(all_urls))

results = []

for i, url in enumerate(all_urls[:MAX_PAGES]):

    print(f"[{i+1}/{min(len(all_urls),MAX_PAGES)}] {url}")
...
with open("all_pages.csv", "w", newline="", encoding="utf-8") as f:
...
print("\nDone.")
print("Pages saved:", len(results))
print("Output: all_pages.json")
print("Output: all_pages.csv")